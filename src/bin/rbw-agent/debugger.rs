...
pub fn disable_tracing() -> anyhow::Result<()> {
    ...
    if ret == 0 {
        ...
        Err(anyhow::anyhow!(
            "failed to disable PTRACE_ATTACH, agent memory may be dumpable by other processes: {e}"
        ))
    }
}
...
pub fn disable_tracing() -> anyhow::Result<()> {
    Err(anyhow::anyhow!(
        "failed to disable PTRACE_ATTACH, agent memory may be dumpable by other processes: unimplemented on this platform"
    ))
}
